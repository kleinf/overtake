...
public class ServerClient extends Thread {
	...
	public void run() {
		...
		try {
			...
			printWriter = new PrintWriter(new DataOutputStream(clientSocket.getOutputStream()));
			final String line = reader.readLine();
			if (line != null) {
				final String[] playerInfos = line.split(Constants.NET_DIVIDER_ESCAPED);
				...
				if (serverId > -1) {
					...
					System.out.println("+ Client " + getClientString() + " hat sich angemeldet!");
					...
					for (String buffer; (buffer = reader.readLine()) != null;) {
						...
						targetid = Integer.parseInt(buffer.substring(0, dividerPos));
					...
					System.out.println("- Client " + getClientString() + " hat sich abgemeldet!");
				} else {
					// Anmeldung ablehnen
	...
	protected void sendError(final String message) {
		printWriter.println(Net.ERROR.name() + Constants.NET_DIVIDER + Net.SERVER.name() + ": " + message);
		printWriter.flush();
	}
	...
	protected String getClientString() {
		return serverId + Constants.NET_DIVIDER + playerName + Constants.NET_DIVIDER + playerColor;
	}
