...
public class BoardPanel extends JPanel implements MouseListener,
	...
	protected BoardPanel(final AbstractMainPanel parentPanel, final Element data) {
		...
		FieldFormatter.getInstance().init(
		...
		relations = new Wall[numFieldsWidth][numFieldsHeight][FieldFormatter
				.getInstance().getMaxRelations()];
		for (int idY = 0; idY < numFieldsHeight; idY++) {
			for (int idX = 0; idX < numFieldsWidth; idX++) {
				if (getFieldComponent(idX, idY) != null) {
					final List<FieldRelation> rel = FieldFormatter
		...
		final double[] pos = FieldFormatter.getInstance().getPosition(
		...
		// Groesse des BoardPanels an Inhalt anpassen
		setSize(FieldFormatter.getInstance().getBoardsize(fieldWidth,
	...
	private void addField(final int idX, final int idY, final int fieldWidth,
		...
		// final Dimension relativeSize = FieldFormatter.getInstance().getSize(
	...
	public void componentResized(final ComponentEvent arg0) {
		createBackground();
		final Dimension relativeSize = FieldFormatter.getInstance()
				.getFieldsize(getWidth(), getHeight(), 0, 0, numFieldsWidth,
						numFieldsHeight);
		for (int idY = 0; idY < GameSession.gameOptions.getNumFieldsHeight(); idY++) {
			for (int idX = 0; idX < GameSession.gameOptions.getNumFieldsWidth(); idX++) {
				final double[] pos = FieldFormatter.getInstance()
						.getPosition(relativeSize.width, relativeSize.height,
								idX, idY, true);
				getFieldComponent(idX, idY).setBounds((int) Math.rint(pos[0]),
						(int) Math.rint(pos[1]), relativeSize.width,
						relativeSize.height);
			}
		}
	}
